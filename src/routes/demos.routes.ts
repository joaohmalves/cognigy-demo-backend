    ...
    try {
      ...
      if (roleName === 'viewer') {
        ...
        if (userDemosError) {
          ...
          return res.status(500).json({
            error:
              'Erro ao buscar demos permitidas',
          });
        }
      ...
      return res.status(403).json({
        error:
          'Role sem permissão para visualizar demos',
      });
    } catch (error) {
      console.error(
